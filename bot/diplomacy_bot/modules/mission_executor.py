...
from . import economy, factory, mission_stats, training, travel
...
def _phase_farm(token: str, cfg: AccountConfig, rt: MissionRuntime, spec: PhaseSpec, *, _api: ApiFn) -> MissionStepResult:
    if travel.is_traveling(token, _api=_api):
        ...
        return _advance_waiting(rt, "travel", int(ts.remaining_ms if ts else 60_000))
    actions: list[dict] = []
    actions.extend(mission_stats.stat_actions(token, cfg, _api=_api))
    ...
    if work.get("ok"):
        actions.extend(mission_stats.stat_actions(token, cfg, suffix="_post", _api=_api))
