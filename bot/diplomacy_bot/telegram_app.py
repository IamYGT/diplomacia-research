...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await _handle_callback(update, context, data, default, query, uid)
    except Exception as e:
...
async def _handle_callback(
    ...
    if data.startswith("nav:account:"):
        ...
        if acc:
            _set_default_account(context, uid, name)
            await _send_dashboard(update, acc, context, edit=True)
        return
    ...
    if data == "dash:refresh":
        acc = _acc()
        if acc:
            await _send_dashboard(update, acc, context, edit=True, force_refresh=True)
        return
