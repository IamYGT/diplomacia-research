...
| # | Epic | Durum | Not |
|---|------|--------|-----|
| 4.1 | Zamanlanmış autopilot job (yeni hesap gelince) | ✅ | `fleet_inbox_watch.py`, `FLEET_INBOX_AUTO_SETUP` varsayılan açık |
...
| 4.14 | Inbox watcher autopilot | ✅ | Yeni token → autopilot varsayılan açık; `FLEET_INBOX_AUTO_SETUP=0` kapatır |
...
| 4.59 | Legacy teknik buton koruması | ✅ | Eski `fleet:tick`, `fleet:af:on`, `role:set` butonları stale ise yan etki üretmez |
| 4.60 | Inbox autopilot varsayılan açık | ✅ | Token dosyası bırakınca worker/watch otomatik autopilot dener; opt-out `FLEET_INBOX_AUTO_SETUP=0` |
| 4.61 | Mission farm stat pre/post | ✅ | Region/AOD farm fazı work öncesi ve sonrası stat otomasyonu çalıştırır |
| 4.62 | Inbox autopilot status görünürlüğü | ✅ | `/fleet status` auto inbox açık/kapalı ve pending token sayısını gösterir |
| 4.63 | Politik faz hata koruması | ✅ | Vatandaşlık/vize/oy API hatası mission fazını sessizce DONE yapmaz |
| 4.64 | DeepSeek plan sözleşmesi | ✅ | Yazılı filo hedefi JSON planına çevrilir; token alanları prompt'a gitmez |
| 4.65 | `/fleetstart` yazılı hedef | ✅ | Doğal cümlede DeepSeek planı dener; kısa komut parser'da kalır |
| 4.66 | `/fleetregion` yazılı hedef | ✅ | Bölge mission da aynı güvenli metin planlayıcıyı kullanır |
| 4.67 | Yazılı hedef görünürlüğü | ✅ | Sonuç mesajında DeepSeek planı veya parser fallback satırı görünür |
| 4.68 | `/fleetplan` önizleme | ✅ | Yazılı hedefi çalıştırmadan plan olarak gösterir |
| 4.69 | Önizleme uygulama komutları | ✅ | `/fleetplan` normalize `/fleetstart` ve `/fleetregion` komutlarını gösterir |
...
# 4. Telegram:
/fleetplan 20 hesabı Hürmüz'e çek oy ver  # çalıştırmadan hedef önizle
...
/fleetregion 20 hesabı Hürmüz'e çek ikamet al oy ver  # doğal bölge mission
/fleetstart 20 hesabı Hürmüz'e çek ana fabrikada çalıştır oy ver  # DeepSeek varsa doğal hedef
/fleetresidence Hürmüz      # toplu ikamet
/fleetvote                  # aktif seçime oy
/fleetcitizen               # vatandaşlık başvurusu (ana ülke)
/autofarm on all

# Otomatik inbox autopilot (varsayılan açık):
# FLEET_INBOX_AUTO_SETUP varsayılan açık; kapatmak için 0 ver
# export FLEET_INBOX_AUTO_SETUP=0

# Opsiyonel yazılı filo planlayıcı:
# export DEEPSEEK_API_KEY=...        # chat'e düşen key sızmış kabul edilip yenilenmeli
...
- [x] Legacy teknik buton koruması: stale `fleet:tick`/`fleet:af:on`/`role:set` yan etki üretmez
- [x] Inbox autopilot varsayılan açık: `FLEET_INBOX_AUTO_SETUP=0` ile opt-out
...
- [x] Region/AOD mission farm fazı work öncesi/sonrası stat otomasyonu çalıştırır
- [x] `/fleet status` auto inbox açık/kapalı ve pending token sayısını gösterir
- [x] Politik mission fazları: vatandaşlık/vize/oy API hatasında faz ilerlemez
- [x] DeepSeek JSON plan sözleşmesi: token/JWT/şifre prompt'a gitmez, unknown action düşer
- [x] `/fleetstart` doğal cümle: DeepSeek plan dener; hata/key yoksa parser fallback
- [x] `/fleetregion` doğal cümle: aynı güvenli planner ile bölge mission kuyruğu
- [x] Yazılı hedef görünürlüğü: DeepSeek planı / parser fallback sonucu mesajda görünür
- [x] `/fleetplan` önizleme: hedefi çalıştırmadan plan/faz olarak gösterir
- [x] `/fleetplan` uygulama komutları: normalize start/region komutları mesajda görünür
...
fleet_inbox_watch.py   — 300s job, otomatik autopilot (varsayılan açık)
...
| Tarih | Sürüm | Not |
|-------|-------|-----|
| 2026-07-01 | 4.28.42 | `/fleetplan` normalize uygulama komutlarını gösterir |
| 2026-07-01 | 4.28.41 | `/fleetplan` yazılı hedef önizleme komutu eklendi |
| 2026-07-01 | 4.28.40 | Yazılı hedef sonuçlarında DeepSeek/fallback görünürlüğü eklendi |
| 2026-07-01 | 4.28.39 | `/fleetregion` doğal cümleyi DeepSeek plan sözleşmesine bağlar |
| 2026-07-01 | 4.28.38 | `/fleetstart` doğal cümleyi DeepSeek plan sözleşmesine bağlar |
| 2026-07-01 | 4.28.37 | DeepSeek yazılı filo hedefi için güvenli JSON plan sözleşmesi eklendi |
| 2026-07-01 | 4.28.36 | Vatandaşlık/vize/oy API hatası durable mission fazını sessizce tamamlamaz |
| 2026-07-01 | 4.28.35 | `/fleet status` auto inbox açık/kapalı ve pending token sayısını gösterir |
| 2026-07-01 | 4.28.34 | Region/AOD mission farm fazı stat pre/post otomasyonunu çalıştırır |
| 2026-07-01 | 4.28.33 | Inbox autopilot varsayılan açık; kapatma opt-out `FLEET_INBOX_AUTO_SETUP=0` |
| 2026-07-01 | 4.28.32 | Eski legacy teknik fleet/rol butonları stale ise yan etki üretmeden status'a yönlenir |
